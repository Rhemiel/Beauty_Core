...
export default function AIAdvisor() {
  ...
  function sendMessage(text) {
    ...
    setTimeout(() => {
      ...
      const aiReply = `Great! Based on your message, I've detected a **${themeLabel}** theme. Here are my top 3 personalized service recommendations for you. You can also generate AI previews of your look using the Advanced AI Generator panel on the right! 🎨`

      setMessages(prev => [...prev, { role:'ai', text: aiReply }])
      setSuggestions({ theme, suggestions: suggs })
      
      // Auto-update generation parameters based on chat
      setGenTheme(themeLabel)
  ...
  function handlePhoto(e) {
    ...
    // Convert to base64 for generation
    ...
    setTimeout(() => {
      setMessages(prev => [...prev, { role:'ai', text:"I've analyzed your photo! ✨ I'll factor this into my recommendations. The uploaded image is now loaded into the Advanced AI Generator on the right for Image-to-Image transformation." }])
      setLoading(false)
      scrollToBottom()
    }, 1500)
  }
  ...
  const generateImages = async () => {
    ...
    setMessages(prev => [...prev, { role: 'ai', text: "Generating your custom AI visual previews based on your parameters..." }]);
    scrollToBottom();
    ...
    try {
      ...
      if (data.images && data.images.length > 0) {
        ...
        setMessages(prev => [...prev, { 
          role: 'ai', 
          text: "Here are your custom AI previews based on your parameters! ✨ Let me know if you want to tweak anything.",
          generatedLooks: urls
        }]);
      }
      ...
      setMessages(prev => [...prev, { role: 'ai', text: `Oops, something went wrong generating the images: ${err.message}` }]);
    } finally {
  ...
  return (
    <div className="flex min-h-screen bg-[#0f0520] font-sans">
      ...
      <div className="ml-[240px] flex-1 flex flex-col">
        ...
        <header className="sticky top-0 z-50 bg-[#1a0a2e] border-b border-[#b040d8]/15 px-8 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="font-sans text-[10px] text-[#9a7ab8] tracking-[1px]">Upload your photo · Chat about your event · Generate Real AI Previews</p>
          </div>
        ...
        <div className="flex flex-1 overflow-hidden" style={{ height:'calc(100vh - 73px)' }}>
            ...
            <div ref={chatRef} className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
              ...
              {!photo && (
                <div className="bg-[#b040d8]/8 border border-dashed border-[#b040d8]/30 p-5 text-center cursor-pointer hover:border-[#f0a800]/50 transition-colors"
                  ...
                  <p className="font-sans text-[10px] text-[#9a7ab8]">Our AI will analyze your features and apply custom looks directly to your face.</p>
                </div>
              )}
              ...
              {messages.map((m, i) => (
                <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`max-w-[75%] ${m.role === 'user' ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                    {m.image && <img src={m.image} alt="uploaded" className="w-32 h-32 object-cover border border-[#b040d8]/30 mb-1 rounded-md" />}
                    
                    {m.generatedLooks && (
                      <div className="flex gap-2 flex-wrap mb-1">
                        {m.generatedLooks.map((url, idx) => (
                          <div key={idx} className="relative group">
                            <img src={url} alt="generated" className="w-48 h-48 object-cover border border-[#b040d8]/30 rounded-md" />
                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center rounded-md">
                              <a href={url} target="_blank" rel="noreferrer" className="text-white text-[10px] uppercase font-bold tracking-[1px] bg-[#7b2fa0] px-3 py-1.5 rounded cursor-pointer no-underline">View Full Size</a>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className={`px-4 py-3 text-[12px] leading-[1.7] rounded-md ${m.role === 'ai'
          ...
          {/* RIGHT: Suggestions & Generation Panel */}
          {suggestions && (
            <div className="w-[380px] shrink-0 border-l border-[#b040d8]/15 overflow-y-auto bg-[#0f0520] custom-scrollbar">
              <div className="p-5">
                {/* Theme badge */}
                ...
                <div className="pt-2 border-t border-[#b040d8]/15 mt-2">
                  <h3 className="font-serif text-[16px] text-[#f5c040] mb-3 tracking-[1px] flex items-center gap-2">✨ Finalize & Generate Previews</h3>
                  
                  {uploadedBase64 && (
                    <div className="mb-3 p-2 bg-[#7b2fa0]/10 border border-[#7b2fa0]/30 rounded-lg flex items-center gap-2">
                      <div className="w-8 h-8 rounded overflow-hidden flex-shrink-0 border border-[#7b2fa0]/40">
                        <img src={photo} alt="Base" className="w-full h-full object-cover" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[10px] text-[#d060f0] font-semibold">Base Image Loaded</p>
                        <p className="text-[8px] text-[#9a7ab8] truncate">IMG2IMG Active</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex flex-col gap-2.5 mb-4">
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="block text-[8px] text-[#9a7ab8] mb-1 uppercase tracking-wider">Style</label>
                      ...
                      <div>
                        <label className="block text-[8px] text-[#9a7ab8] mb-1 uppercase tracking-wider">Color Palette</label>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="block text-[8px] text-[#9a7ab8] mb-1 uppercase tracking-wider">Theme</label>
                      ...
                      <div>
                        <label className="block text-[8px] text-[#9a7ab8] mb-1 uppercase tracking-wider">Event / Occasion</label>
                    ...
                    <div>
                      <label className="block text-[8px] text-[#9a7ab8] mb-1 uppercase tracking-wider">Design Elements</label>
                    ...
                    <button onClick={generateImages} disabled={isGenerating} className="w-full py-3 mt-1 bg-gradient-to-r from-[#7b2fa0] to-[#f0a800] text-white font-sans text-[11px] font-bold tracking-[1px] uppercase border-none cursor-pointer disabled:opacity-50 hover:opacity-90 shadow-lg shadow-[#7b2fa0]/20 flex items-center justify-center gap-2">
                      {isGenerating ? <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></span> : '✨ Generate AI Previews'}
                    </button>
                    {error && <p className="text-red-400 text-[9px] mt-1 p-2 bg-red-400/10 border border-red-400/30 rounded">{error}</p>}
                  </div>

                  <div className="flex flex-col gap-3 pb-6">
                    {isGenerating ? (
                      <div className="flex justify-center p-8"><span className="w-8 h-8 border-[3px] border-[#f0a800] border-t-transparent rounded-full animate-spin"></span></div>
                    ) : generatedImages.length > 0 ? (
                      <div className="grid grid-cols-1 gap-3">
                        {generatedImages.map((url, i) => (
                          <div key={i} className="border border-[#b040d8]/15 overflow-hidden rounded-md relative group">
                      ...
                      <div className="border border-dashed border-[#b040d8]/30 p-8 text-center text-[#9a7ab8] text-[11px] rounded-md">
                        No AI previews generated yet.<br/>Adjust the styling parameters and click Generate.
                      </div>
                    )}
                  </div>
                </div>
      ...
      {/* Scrollbar styles for the right panel */}
